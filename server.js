...
app.get('/api/exchange-rate/:from/:to', async (req, res) => {
  try {
    ...
    // 使用備用匯率
    const fallbackRate = getFallbackRate(req.params.from, req.params.to);
    res.json({
      success: true,
      rate: fallbackRate,
      fallback: true
...
function getFallbackRate(from, to) {
  const fallbackRates = {
    'USD': { 'TWD': 31.5, 'JPY': 150, 'EUR': 0.92, 'CNY': 7.2, 'KRW': 1300, 'HKD': 7.8, 'SGD': 1.35, 'THB': 35.5, 'VND': 24500, 'MYR': 4.7, 'PHP': 56, 'IDR': 15500, 'GBP': 0.79, 'CAD': 1.36, 'AUD': 1.52, 'CHF': 0.88 },
    'TWD': { 'USD': 0.032, 'JPY': 4.76, 'EUR': 0.029, 'CNY': 0.23, 'KRW': 41.3, 'HKD': 0.25, 'SGD': 0.043, 'THB': 1.13, 'VND': 778, 'MYR': 0.15, 'PHP': 1.78, 'IDR': 492, 'GBP': 0.025, 'CAD': 0.043, 'AUD': 0.048, 'CHF': 0.028 },
    'EUR': { 'USD': 1.09, 'TWD': 34.4, 'JPY': 163, 'CNY': 7.8, 'KRW': 1420, 'HKD': 8.5, 'SGD': 1.47, 'THB': 38.7, 'VND': 26700, 'MYR': 5.1, 'PHP': 61, 'IDR': 16900, 'GBP': 0.86, 'CAD': 1.48, 'AUD': 1.66, 'CHF': 0.96 }
  };
  ...
  // 通過 USD 轉換
  if (fallbackRates[from] && fallbackRates[from]['USD'] && 
      fallbackRates['USD'] && fallbackRates['USD'][to]) {
